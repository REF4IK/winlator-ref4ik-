...
public class WineInfo implements Parcelable {
    ...
    public static WineInfo fromIdentifier(Context context, ContentsManager contentsManager, String identifier) {
        ...
        if (wineProfile != null && wineProfile.type == ContentProfile.ContentType.CONTENT_TYPE_WINE) {
            String verName = wineProfile.verName != null ? wineProfile.verName.toLowerCase() : "";
            path = ContentsManager.getInstallDir(context, wineProfile).getPath();

            Matcher profileMatcher = pattern.matcher(verName);
            if (profileMatcher.find()) {
                return new WineInfo(profileMatcher.group(1), profileMatcher.group(2), profileMatcher.group(4), path);
            }

            String arch = "x86_64";
            if (verName.contains("arm64ec")) arch = "arm64ec";
            else if (verName.contains("x86_64")) arch = "x86_64";
            else if (verName.contains("x86")) arch = "x86";

            String type = "wine";
            if (verName.startsWith("proton-")) type = "proton";

            String version = wineProfile.verName != null ? wineProfile.verName : "unknown";
