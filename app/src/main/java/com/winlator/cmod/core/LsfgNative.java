...
public final class LsfgNative {
    ...
    /** Shared global Lossless.dll (Settings import target, engine source). */
    ...
    public static String globalDllPath(Context context) {
        File f = globalDllFile(context);
        return f != null && f.isFile() && f.length() > 0 ? f.getAbsolutePath() : null;
    }
    ...
    public static boolean importGlobalLosslessDll(Context context, Uri uri) {
        ...
        try (InputStream in = context.getContentResolver().openInputStream(uri);
             OutputStream out = new java.io.FileOutputStream(dllFile)) {
            if (in == null) return false;
            byte[] buf = new byte[8192];
            int n;
            while ((n = in.read(buf)) > 0) out.write(buf, 0, n);
            FileUtils.chmod(dllFile, 0644);
            Log.i(TAG, "Imported global Lossless.dll, size=" + dllFile.length());
