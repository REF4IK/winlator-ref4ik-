...
public abstract class AppUtils {
    ...
    public static void hideSystemUI(final Activity activity) {
        ...
        // Enable display cutout support for devices with notch/camera cutout
    ...
    public static PopupWindow showPopupWindow(View anchor, View contentView, int width, int height) {
        int widthPx = width > 0 ? (int)UnitUtils.dpToPx(width) : 0;
        int heightPx = height > 0 ? (int)UnitUtils.dpToPx(height) : 0;
        return showPopupWindowPx(anchor, contentView, widthPx, heightPx);
    }

    private static PopupWindow showPopupWindowPx(View anchor, View contentView, int widthPx, int heightPx) {
        ...
        int screenPadding = (int)UnitUtils.dpToPx(12);
        int maxPopupWidth = Math.max(1, Math.min(screenWidth, windowFrame.width()) - screenPadding * 2);
        int maxPopupHeight = Math.max(1, windowFrame.height() - screenPadding * 2);

        int desiredWidth = widthPx > 0 ? Math.min(widthPx, maxPopupWidth) : 0;
        int desiredHeight = heightPx > 0 ? Math.min(heightPx, maxPopupHeight) : 0;

        int widthMeasureSpec = desiredWidth > 0
        ...
        int heightMeasureSpec = desiredHeight > 0
        ...
        int popupWidth = desiredWidth > 0 ? desiredWidth : Math.min(contentView.getMeasuredWidth(), maxPopupWidth);
        int popupHeight = desiredHeight > 0 ? desiredHeight : Math.min(contentView.getMeasuredHeight(), maxPopupHeight);

        popupWindow.setWidth(Math.max(1, popupWidth));
        popupWindow.setHeight(Math.max(1, popupHeight));
        ...
        int availableBelow = Math.max(1, windowFrame.bottom - (anchorY + anchor.getHeight()) - screenPadding);
        int availableAbove = Math.max(1, anchorY - windowFrame.top - screenPadding);

        boolean showAbove = popupHeight > availableBelow && availableAbove > availableBelow;
        ...
        int xOffset = 0;
        if (anchorX + popupWidth > screenWidth - screenPadding) {
            xOffset = (screenWidth - screenPadding) - (anchorX + popupWidth);
        }
        if (anchorX + xOffset < screenPadding) {
            xOffset = screenPadding - anchorX;
        }
        int yOffset = showAbove ? -(anchor.getHeight() + popupHeight) : 0;
    ...
    public static void showHelpBox(Context context, View anchor, String text) {
        showHelpBox(context, anchor, text, 300);
    }

    public static void showHelpBox(Context context, View anchor, int textResId, int popupWidthDp) {
        showHelpBox(context, anchor, context.getString(textResId), popupWidthDp);
    }

    public static void showHelpBox(Context context, View anchor, String text, int popupWidthDp) {
        int padding = (int)UnitUtils.dpToPx(8);
        int screenWidth = context.getResources().getDisplayMetrics().widthPixels;
        int popupWidthPx = Math.min((int)UnitUtils.dpToPx(popupWidthDp), screenWidth - (int)UnitUtils.dpToPx(24));
        ...
        scrollView.setLayoutParams(new ViewGroup.LayoutParams(popupWidthPx, ViewGroup.LayoutParams.WRAP_CONTENT));
        scrollView.setFillViewport(true);
        scrollView.addView(textView);

        showPopupWindowPx(anchor, scrollView, popupWidthPx, 0);
    }
