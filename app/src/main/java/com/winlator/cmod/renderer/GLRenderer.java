...
public class GLRenderer implements GLSurfaceView.Renderer, WindowManager.OnWindowModificationListener, Pointer.OnPointerMotionListener, HostRenderer, XServerRenderer {
    ...
    // is P4 вЂ” until then the game is still composited by GL; the opaque game SC is bufferless so it
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        // native mode is active вЂ” mirrors VulkanRenderer.onSurfaceCreated's nativeMode restore block.
    ...
    private void renderDrawable(Drawable drawable, int x, int y, ShaderMaterial material) {
        ...
        synchronized (drawable.renderLock) {
            ...
            // stays smooth. This is sampler state only вЂ” it does NOT touch the GPUImage upload path.
    ...
    // exactly, minus the cursor вЂ” so the pointer is NEVER rendered into the low-res target
    ...
    // Applies the screen effects (Vulkan-compatible effect codes + params) via the EffectComposer.
    public void setEffects(int[] types, float[][] paramsArr) { effectComposer.setEffects(types, paramsArr); }
    public void clearEffects() { effectComposer.clearEffects(); }

    // ---- XServerRenderer stubs (DrawableManager/legacy expects these) ----
    @Override public XServerView getRendererView() { return xServerView; }
    @Override public void onSurfaceCreated(android.view.Surface surface) {}
    @Override public void onSurfaceChanged(int width, int height) { onSurfaceChanged(null, width, height); }
    @Override public void onUpdateWindowContentDirect(Window window, Drawable drawable, short xOff, short yOff) { xServerView.requestRender(); }
    @Override public void setOnFrameRenderedListener(Runnable listener) {}
    @Override public String getForceFullscreenWMClass() { return null; }
    @Override public void setForceFullscreenWMClass(String wmClass) {}
    @Override public boolean isNativeMode() { return nativeMode; }
    ...
    /** FPS-limiter cap when set, else the display refresh rate вЂ” the frame-rate hint fed to the
    ...
    // ---- GL Native Rendering (direct scanout) lifecycle вЂ” mirrors VulkanRenderer ----
    ...
    // Feed the destination (on-screen) rect to the scanout context вЂ” the letterbox/fullscreen mapping
     ...
     * overlay (the whole point вЂ” SurfaceFlinger skips GL composition). The FLIP path bypasses the GL
    ...
    public void presentScanout(Window window, Drawable content) {
        ...
        synchronized (content.renderLock) {
            ...
            g.lock();
            content.setData(g.getVirtualData());
