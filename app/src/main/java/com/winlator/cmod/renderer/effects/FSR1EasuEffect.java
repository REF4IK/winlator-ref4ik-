 ...
 *
 * Edge-Adaptive Spatial Upsampling. Reconstructs sharp edges from a lower
 * resolution input texture. Should be paired with {@link FSR1RcasEffect} for
 * a final sharpening pass.
 *
 * Ported to the cmod GLES post-process pipeline from the official GPUOpen source:
 * https://github.com/GPUOpen-Effects/FidelityFX-FSR/blob/master/ffx-fsr/ffx_fsr1.h
 *
 * Copyright (c) 2021 Advanced Micro Devices, Inc.  Released under the MIT license.
 */
public class FSR1EasuEffect extends Effect implements RenderScaleEffect {
    /** AMD-recommended FSR 1.0 quality presets (input scale relative to output). */
    public enum Quality {
        ULTRA_QUALITY(1.3f),
        QUALITY(1.5f),
        BALANCED(1.7f),
    ...
    private Quality quality = Quality.QUALITY;
    private boolean preserveAspect = false;

    public Quality getQuality() { return quality; }
    public void setQuality(Quality quality) { if (quality != null) this.quality = quality; }

    public boolean isPreserveAspect() { return preserveAspect; }
    public void setPreserveAspect(boolean preserveAspect) { this.preserveAspect = preserveAspect; }
    ...
    public void onUse(ShaderMaterial material, GLRenderer renderer) {
        int outputW = Math.max(1, renderer.getSurfaceWidth());
        int outputH = Math.max(1, renderer.getSurfaceHeight());
        int inputW = getRenderWidth(renderer, outputW);
        int inputH = getRenderHeight(renderer, outputH);
        material.setUniformVec2("inputResolution", inputW, inputH);
        material.setUniformVec2("outputResolution", outputW, outputH);
    ...
    public int getRenderWidth(GLRenderer renderer, int outputWidth) {
        int byQuality = Math.max(1, (int) Math.round(outputWidth / quality.scale));
        // Never render larger than the X server output (game's logical resolution).
        return Math.max(1, Math.min(byQuality, renderer.getXServerWidth()));
    }

    @Override
    public int getRenderHeight(GLRenderer renderer, int outputHeight) {
        int byQuality = Math.max(1, (int) Math.round(outputHeight / quality.scale));
        return Math.max(1, Math.min(byQuality, renderer.getXServerHeight()));
    }

    private static class FSR1EasuMaterial extends ScreenMaterial {
        public FSR1EasuMaterial() {
            super();
            setUniformNames("screenTexture", "resolution", "inputResolution", "outputResolution", "preserveAspect");
        }
