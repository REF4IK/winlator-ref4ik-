...
public class Texture {
    ...
    public void updateFromDrawable(Drawable drawable) {
        ...
        if (!isAllocated()) {
            ...
            drawable.clearDirtyRegion();
        }
        else if (needsUpdate) {
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, textureId);
            if (drawable.hasDirtyRegion()) {
                short dx = drawable.getDirtyX();
                short dy = drawable.getDirtyY();
                short dw = drawable.getDirtyWidth();
                short dh = drawable.getDirtyHeight();
                // Clamp dirty region to drawable bounds
                if (dx < 0) { dw += dx; dx = 0; }
                if (dy < 0) { dh += dy; dy = 0; }
                if (dx + dw > drawable.width) dw = (short)(drawable.width - dx);
                if (dy + dh > drawable.height) dh = (short)(drawable.height - dy);
                if (dw > 0 && dh > 0) {
                    // For partial upload we need to set unpack parameters
                    GLES20.glPixelStorei(GLES20.GL_UNPACK_ALIGNMENT, unpackAlignment);
                    // Upload only the dirty sub-region
                    // We need to position the data pointer to the start of the dirty region
                    int rowBytes = drawable.width * 4;
                    int offset = (dy * drawable.width + dx) * 4;
                    data.position(offset);
                    // If dirty region spans the full width, do a single glTexSubImage2D
                    if (dx == 0 && dw == drawable.width) {
                        GLES20.glTexSubImage2D(GLES20.GL_TEXTURE_2D, 0, dx, dy, dw, dh, format, GLES20.GL_UNSIGNED_BYTE, data);
                    } else {
                        // Upload row by row for sub-width regions
                        for (int row = 0; row < dh; row++) {
                            data.position(((dy + row) * drawable.width + dx) * 4);
                            GLES20.glTexSubImage2D(GLES20.GL_TEXTURE_2D, 0, dx, (short)(dy + row), dw, (short)1, format, GLES20.GL_UNSIGNED_BYTE, data);
                        }
                    }
                    data.rewind();
                }
                ...
                // Fallback: upload entire texture
