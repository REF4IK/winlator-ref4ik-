...
public class AdrenotoolsFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View btDriverStore = layout.findViewById(R.id.BTDriverStore);
        ...
        btDriverStore.setOnClickListener((v) -> {
            // Открываем фрагмент магазина драйверов
            DriverStoreFragment driverStoreFragment = new DriverStoreFragment();
            getActivity().getSupportFragmentManager()
                    .beginTransaction()
                    .replace(R.id.FLFragmentContainer, driverStoreFragment)
                    .addToBackStack(null)
                    .commit();
        });
        return layout;
    ...
    private class DriversAdapter extends RecyclerView.Adapter<DriversAdapter.ViewHolder> {
        ...
        public class ViewHolder extends RecyclerView.ViewHolder {
            ...
            public ViewHolder(View v) {
                ...
                btExport = v.findViewById(R.id.BTExport);
            }
        }
        ...
        public void onBindViewHolder(ViewHolder viewHolder, final int position) {
            String driverId = driversList.get(position);
            viewHolder.tvName.setText(adrenotoolsManager.getDriverName(driverId));
            viewHolder.tvVersion.setText(adrenotoolsManager.getDriverVersion(driverId));
            viewHolder.btMenu.setOnClickListener((v) -> {
                removeAtIndex(position);
            });

            if (viewHolder.btExport != null) {
                viewHolder.btExport.setOnClickListener(v -> {
                    try {
                        boolean ok = adrenotoolsManager.exportDriverToDownloads(driverId);
                        if (ok) {
