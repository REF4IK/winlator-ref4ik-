...
public class WinHandler {
    ...
    public void updateGyroData(float rawGyroX, float rawGyroY) {
        ...
        // If gyro is enabled in settings, always set isGyroActive to true
        // This allows gyro to work without needing to press a button
        if (gyroEnabled && !processGyroWithLeftTrigger) {
            isGyroActive = true;
        }

        if (isGyroActive && shouldProcessGyro) {
            // Apply deadzone
            if (Math.abs(rawGyroX) < gyroDeadzone) rawGyroX = 0;
            if (Math.abs(rawGyroY) < gyroDeadzone) rawGyroY = 0;

            // Apply inversion - invert X axis for correct left/right movement
            // Keep Y axis as is for correct up/down movement
            rawGyroX = -rawGyroX;
            // rawGyroY stays as is (not inverted)

            // Also apply user preference inversion
            if (invertGyroX) rawGyroX = -rawGyroX;
            if (invertGyroY) rawGyroY = -rawGyroY;

            // Get sensitivity from preferences, default to 1.0 if not set
            float userSensitivity = preferences.getFloat("gyro_sensitivity", 1.0f);
            float sensitivityMultiplier = 5.0f * userSensitivity; // Base multiplier multiplied by user sensitivity
            ...
            // Send mouse movement instead of gamepad state
            int dx = (int) smoothGyroX;
            int dy = (int) smoothGyroY;
...
//            currentController.setTriggerType(triggerType); // Ensure triggerType is set
    ...
    public void showDesktop() {
        // Send a command to minimize all windows
        exec("explorer.exe /minall");
    }
}
