...
public class TiledBackgroundView extends View {
    ...
    private void loadAnimationFrames(String animationBaseName) {
        ...
        // If no frames loaded, create a simple fallback bitmap
        if (!hasValidFrames) {
            Bitmap fallbackBitmap = Bitmap.createBitmap(100, 100, Bitmap.Config.ARGB_8888);
            fallbackBitmap.eraseColor(0xFF1a1a1a); // Dark gray fallback
            frames[0] = fallbackBitmap;
        }
    ...
    private void updateShader() {
        if (frames == null || frames.length == 0 || frames[currentFrame] == null) {
            // Create a fallback if shader update fails
            paint = new Paint();
            paint.setColor(0xFF1a1a1a); // Dark gray fallback
            return;
        }
    ...
    private final Runnable animationRunnable = new Runnable() {
        ...
        public void run() {
            if (isAnimating && frames != null && frames.length > 0) {
                ...
                if (enableParallax && frames[currentFrame] != null) { // Only update scroll if parallax is enabled
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (paint == null) {
            // Paint not initialized yet, skip drawing
            return;
        }
        ...
        if (staticWallpaper != null) {
            ...
            if (paint.getShader() != null) {
                ...
                // Fallback to solid color if no shader available
