...
//            }
    ...
    public boolean extractContainerPatternFile(Container container, String wineVersion, ContentsManager contentsManager, File containerDir, OnExtractFileListener onExtractFileListener) {
        ...
        if (profile != null && profile.type == ContentProfile.ContentType.CONTENT_TYPE_WINE) {
            String prefixPack = profile.winePrefixPack != null && !profile.winePrefixPack.isEmpty() ? profile.winePrefixPack : "prefix.tar.xz";
            File containerPatternFile = new File(ContentsManager.getInstallDir(context, profile), prefixPack);

            if (!containerPatternFile.exists()) {
                containerPatternFile = new File(ContentsManager.getInstallDir(context, profile), "prefixPack.txz");
            }
            if (!containerPatternFile.exists()) {
                containerPatternFile = new File(ContentsManager.getInstallDir(context, profile), "prefixPack.tar.xz");
            }
            if (!containerPatternFile.exists()) {
                containerPatternFile = new File(ContentsManager.getInstallDir(context, profile), "prefixPack.tar.zst");
            }

            String name = containerPatternFile.getName().toLowerCase();
            TarCompressorUtils.Type type = name.endsWith(".zst") || name.endsWith(".zstd") || name.endsWith(".tzst")
