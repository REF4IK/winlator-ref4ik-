...
public class SeekBar extends AppCompatImageView {
    ...
    public SeekBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (context.getTheme().resolveAttribute(com.google.android.material.R.attr.colorOnSurface, outValue, true)) {
            if (outValue.resourceId != 0) {
                textColor = ContextCompat.getColor(context, outValue.resourceId);
            } else {
                textColor = outValue.data;
            }
        } else {
            textColor = -0x8c8c8d; // Dark gray fallback
        }
