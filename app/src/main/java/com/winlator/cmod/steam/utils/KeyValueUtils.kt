...
import com.winlator.cmod.steam.data.BranchInfo
import com.winlator.cmod.steam.data.ConfigInfo
import com.winlator.cmod.steam.data.DepotInfo
import com.winlator.cmod.steam.data.LaunchInfo
import com.winlator.cmod.steam.data.LibraryAssetsInfo
import com.winlator.cmod.steam.data.LibraryCapsuleInfo
import com.winlator.cmod.steam.data.LibraryHeroInfo
import com.winlator.cmod.steam.data.LibraryLogoInfo
...
import com.winlator.cmod.steam.data.UFS
...
import com.winlator.cmod.steam.service.SteamService.Companion.INVALID_APP_ID
...
fun KeyValue.generateSteamApp(): SteamApp {
    return SteamApp(
        ...
        dlcAppIds = this["extended"]["listofdlc"].value?.split(",")?.mapNotNull { it.trim().toIntOrNull() } 
            ?: this["common"]["extended"]["listofdlc"].value?.split(",")?.mapNotNull { it.trim().toIntOrNull() } 
            ?: emptyList(),
        ...
        ufs = run {
            ...
            UFS(
                ...
                saveFilePatterns = this["ufs"]["savefiles"].children.mapNotNull { saveFile ->
                    val platforms = saveFile["platforms"].children.mapNotNull { it.value?.lowercase() }
                    ...
                    SaveFilePattern(
                        ...
                        path = rootRemap?.let { rootOverride ->
                            var adjustedPath = if (rootOverride.addPath.isNotEmpty()) {
                                val normalizedAddPath = rootOverride.addPath.replace('\\', '/').trimEnd('/')
                                if (originalPath.isNotEmpty()) {
                                    "$normalizedAddPath/${originalPath.trimStart('/')}"
                                } else {
                                    normalizedAddPath
                                }
                            } else {
                            ...
                            rootOverride.pathTransforms.forEach { (find, replace) ->
                                adjustedPath = adjustedPath.replace(find, replace)
                            }
