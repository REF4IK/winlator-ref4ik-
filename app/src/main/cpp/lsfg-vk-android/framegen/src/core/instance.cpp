...
Instance::Instance() {
    ...
    this->ownsHandle_ = true;
}

Instance::Instance(VkInstance existingInstance) {
    this->instance = std::shared_ptr<VkInstance>(
        new VkInstance(existingInstance),
        [](VkInstance*) {} // no-op — caller owns the instance
    );
