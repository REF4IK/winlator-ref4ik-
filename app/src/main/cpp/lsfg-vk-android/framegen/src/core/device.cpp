...
// External device constructor — wraps an existing VkDevice (single-device mode).
// Does NOT create a new device, just stores handles. No ownership.
Device::Device(VkDevice existingDevice, VkPhysicalDevice physDev,
               uint32_t computeFamilyIdx, VkQueue computeQueue,
               bool nullDescriptorSupported)
    : physicalDevice(physDev)
    , computeFamilyIdx(computeFamilyIdx)
    , computeQueue(computeQueue)
    , nullDescriptorSupported(nullDescriptorSupported)
    , ownsHandle_(false)
{
    LSFG_FRAMEGEN_LOGI("Entering Device::Device (external, single-device mode) — %s", kFramegenBuildStamp);

    this->device = std::shared_ptr<VkDevice>(
        new VkDevice(existingDevice),
        [](VkDevice*) {} // no-op — caller owns the device
    );

    // Create fallback descriptor image for null descriptor support
