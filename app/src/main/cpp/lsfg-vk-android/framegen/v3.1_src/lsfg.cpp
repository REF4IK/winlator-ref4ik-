...
void LSFG_3_1::initializeExternal(VkDevice existingDevice, VkPhysicalDevice physicalDevice,
    ...
    // Create an external instance wrapper (no-op destroy).
    // Not strictly needed for device creation since we pass handles directly,
    // but kept for API compatibility — instance.handle() may be called downstream.
    instance.emplace(VK_NULL_HANDLE);

    device.emplace(Vulkan {
        .device{existingDevice, physicalDevice, computeFamilyIdx, computeQueue},
        .generationCount = generationCount,
        .flowScale = flowScale,
        .isHdr = isHdr
    });
