...
namespace LSFG_3_1 {
    ...
    ///
    /// Initialize the LSFG library using an existing VkDevice (single-device mode).
    /// Use this when running inside a Vulkan layer that already has a device.
    ///
    /// @param existingDevice The existing Vulkan device to use for compute.
    /// @param physicalDevice The physical device backing the logical device.
    /// @param computeFamilyIdx Queue family index for compute operations.
    /// @param computeQueue Queue handle for compute submissions.
    /// @param isHdr Whether the images are in HDR format.
    /// @param flowScale Internal flow scale factor.
    /// @param generationCount Number of frames to generate.
    /// @param loader Function to load shader source code by name.
    ///
    __attribute__((visibility("default")))
    void initializeExternal(VkDevice existingDevice, VkPhysicalDevice physicalDevice,
