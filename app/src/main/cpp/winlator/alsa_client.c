...
static AAudioStream *aaudioCreateVirtual(int32_t format, int8_t channelCount, int32_t sampleRate, int32_t bufferSize) {
    aaudio_result_t result;
    AAudioStreamBuilder *builder;
    AAudioStream *stream;
    ...
    // Configure as a virtual stream (silent, for timing only)
    AAudioStreamBuilder_setPerformanceMode(builder, AAUDIO_PERFORMANCE_MODE_LOW_LATENCY);
    AAudioStreamBuilder_setFormat(builder, toAAudioFormat(format));
    AAudioStreamBuilder_setChannelCount(builder, channelCount);
    AAudioStreamBuilder_setSampleRate(builder, sampleRate);
    AAudioStreamBuilder_setSharingMode(builder, AAUDIO_SHARING_MODE_SHARED); // Use shared mode for virtual
    ...
    __android_log_print(ANDROID_LOG_DEBUG, TAG, "Created virtual ALSA stream for reflector mode");
    return stream;
}
...
static int aaudioWriteVirtual(AAudioStream *aaudioStream, void *buffer, int numFrames) {
    // For virtual streams, we simulate the write operation without actual audio output
    // This provides timing synchronization for the reflector mode
    
    if (aaudioStream == NULL) return -1;
    
    // Simulate processing delay
    usleep(1000); // 1ms delay to simulate processing
    
    __android_log_print(ANDROID_LOG_VERBOSE, TAG, "Virtual write: %d frames", numFrames);
    return numFrames; // Always return success for virtual writes
}
...
JNIEXPORT jlong JNICALL
Java_com_winlator_cmod_alsaserver_ALSAClient_createVirtualStream(JNIEnv *env, jobject obj, jint format,
                                               jbyte channelCount, jint sampleRate, jint bufferSize) {
    return (jlong)aaudioCreateVirtual(format, channelCount, sampleRate, bufferSize);
}
...
JNIEXPORT jint JNICALL
Java_com_winlator_cmod_alsaserver_ALSAClient_writeVirtual(JNIEnv *env, jobject obj, jlong streamPtr, jobject buffer,
                                              jint numFrames) {
    AAudioStream *aaudioStream = (AAudioStream*)streamPtr;
    if (aaudioStream) {
        return aaudioWriteVirtual(aaudioStream, (*env)->GetDirectBufferAddress(env, buffer), numFrames);
    }
    else return -1;
}
